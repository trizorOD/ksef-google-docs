...
// Column order for Sprzedaż (A..S) after the schema change:
...
// never written), Reminder sent, Overdue email sent, Final notice sent
...
test('parseSaleRowsForReminders reads the tracking columns when already sent', () => {
  const raw = [['Invoice'], saleRow({ 16: '05-09-2026', 17: '', 18: '12-09-2026' })];
  const rows = parseSaleRowsForReminders(raw);
  assert.equal(rows[0].reminderSent, '05-09-2026');
  assert.equal(rows[0].overdueSent, '');
  assert.equal(rows[0].finalNoticeSent, '12-09-2026');
});
...
test('computeMissingHeaders appends after a real-world extra manual column ("Overdue days") without touching it', () => {
  ...
  // managed by this code. The repair must append the tracking headers
  // starting at column Q (index 16), never touch column P.
  const current = [
  ...
  const expected = [...current.slice(0, 15), 'Overdue days', 'Reminder sent', 'Overdue email sent', 'Final notice sent'];
  const diff = computeMissingHeaders(current, expected);
  assert.deepEqual(diff, { startColIndex: 16, missing: ['Reminder sent', 'Overdue email sent', 'Final notice sent'] });
});
